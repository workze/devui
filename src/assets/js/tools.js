...
* Copyright (c) 2015 Nathan Bubna; Licensed MIT */
		...
		for (i = 0; i < zte_headers.length; i++) {
			if (zte_headers[i].store === true) {
				if ( !! store(zte_headers[i].value)) {
					request.headers[zte_headers[i].key] = escape(store(zte_headers[i].value));
				}
			} else {
				request.headers[zte_headers[i].key] = escape(zte_headers[i].value);
			}
		}
	}
	if(store('language-option') && store('language-option') != "null"){
		request.headers["language-option"] = store('language-option');
	}else{
		request.headers["language-option"] = "";
	}		
});
/**
 * 初始化脚本文件装载工具
...
function modelTip(title, message, confirm, cancel){	
	if(top != window){
		top.modelTip(title, message, confirm, cancel);
		return;
	}
	if(top.$('#frmModal').length == 0){ //当前页面是顶级页面
		var frmModalHtml = 
		  ...
		  '<div class="modal-dialog">' + 
			'<div class="modal-content">' + 
			  '<div class="modal-header">' + 
				...
				'<h4 class="modal-title" id="frmModalTitle">Modal title</h4>' +
			  '</div>' + 
			  '<div class="modal-body"></div>' + 
			  '<div class="modal-footer">' + 
				'<button type="button" class="btn btn-default" id="frmCancel" data-dismiss="modal">Cancel</button>' + 
				'<button type="button" class="btn btn-primary" id="frmConfirm">OK</button>' + 
			  '</div>' + 
			'</div>' + 
		  '</div>' +
		'</div>';
		$("body").append(frmModalHtml);
	}
	if(title){
		$("#frmModalTitle").text(title);
	}else{
		$("#frmModalTitle").hide();
		$("#frmModal .modal-header").css("border-bottom", "0px");
	}
	$("#frmModal .modal-body").html(message);
	$("#frmModal #frmConfirm").html($.i18n.prop("com_zte_ums_ict_sm_user_confirm"));
	$("#frmModal #frmCancel").html($.i18n.prop("com_zte_ums_ict_sm_user_cancel_button"));
	$('#frmModal').modal({
		keyboard: false,
		backdrop: false
	});
...
function showTipBox(level, message){		
	hideTipBox();
	if(level == 0){
	...
	$("#com_zte_ums_ict_portal_frame_tip").html(message);
	if(level > 2){
		setTimeout(function(){
			hideTipBox();
		}, 3000);
...
function hideTipBoxNew(index){
	/* $("#frmTipBox_" + project-home).removeClass("frmTipBoxTransition");
	$("#frmTipBox_" + project-home).removeClass('in');
	$("#frmTipBox_" + project-home).removeClass(alertLevel);
	$("#com_zte_ums_ict_portal_frame_tip_" + project-home).html(""); */
...
//每次产生新的tip box
function showTipBoxNew(level, message){	
	//框架统一提示框	
	var tipBoxIndex = generateUUID();	
	var tipBoxHtml = '<div class="alert fade row frmTipBox" id="frmTipBox_' + tipBoxIndex + '">' +
			'<button class="close" data-close="alert" onclick=\'hideTipBoxNew("' + tipBoxIndex + '");\'></button>' +
			'<div id="com_zte_ums_ict_portal_frame_tip_' + tipBoxIndex + '" style="display:inline"></div>' + 
		'</div>';
	
	var cssTop = 24;
	var initScrollTop = $(window).scrollTop();
	var frmBoxes = $(".frmTipBox");
	if(frmBoxes.length == 0){
		cssTop = initScrollTop + 24;
	}else{
		cssTop = $(frmBoxes[frmBoxes.length - 1]).offset().top + $(frmBoxes[frmBoxes.length - 1]).height() + 20;
	}
	
	$("body").append(tipBoxHtml);
	$("#frmTipBox_" + tipBoxIndex).offset({top:cssTop});
	...
	$(window).scroll(function() {
		$("#frmTipBox_" + tipBoxIndex).css("top", $(window).scrollTop() + (cssTop - initScrollTop) + "px");
	});
	...
	$("#frmTipBox_" + tipBoxIndex).addClass(alertLevel);
	$("#frmTipBox_" + tipBoxIndex).addClass('in', 1000);
	$("#frmTipBox_" + tipBoxIndex).addClass("frmTipBoxTransition");	
	$("#com_zte_ums_ict_portal_frame_tip_" + tipBoxIndex).html(message);
	if(level > 2){
		setTimeout(function(){
			hideTipBoxNew(tipBoxIndex);
		}, 3000);
	}	
...
function getAllOperCodeRights(operations) {
    ...
    var data = {"operations" : operations};
    $.ajax({
        "async": false,
        "type" : "POST",
        ...
        "success" : function (resp) {				
            var hasRightOP = resp["operations"];
            var len = operations.length;
			var username = store("username");
	
            for(var i=0;i<len;i++){
                if(hasRightOP.indexOf(operations[i]) > -1){
                    rights.push(true);
